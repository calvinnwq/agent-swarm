...
- Use `agent-swarm init` for first-time minimal project config; it writes only `.agent-swarm/config.yml`, preserves existing config unless `--force` is passed, and CLI flags still override every value.
...
2. Create only the needed directories: `.agent-swarm/agents/`, `.agent-swarm/presets/`, and optionally `.agent-swarm/config.yml`; prefer `agent-swarm init` for the default config skeleton. Group agent and preset files in subdirectories when it improves readability.
