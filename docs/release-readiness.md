...
The static docs site is published by `.github/workflows/pages.yml`: pushes to `main` that touch `docs/site/**` or the workflow upload `docs/site/` directly to GitHub Pages with no build step. Preview the same `docs/site/` tree locally with `pnpm docs:serve` (or `pnpm docs:serve -- --port 4000` / `PORT=4000 pnpm docs:serve`). The site contract is covered by `test/unit/docs-site.test.ts` as part of `pnpm test`.
...
### M15 Runtime Boundary Contract Guardrail
...
Result:  architecture contract suite passed
```
