...
`runSwarm` (in `src/lib/run-swarm.ts`) sets up fresh runs, while `resumeSwarm`
rehydrates interrupted runs. Both hand the shared runtime core to `executeRun`.
The lifecycle:
